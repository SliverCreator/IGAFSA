function [ nextPosition,nextPositionH ] = GridAF_follow( n,N,ppValue,ii,visual,delta,try_number,lastH,Barrier,goal, j, MAXGEN,rightInf,BX,a)
%输入参数：
% n----矩阵维数
% N----人工鱼总数
% ppValue----所有人工鱼群的位置
...
%-------------------------------------------------------------------------------------------------------------    
    ...
    if Hmin/Nf <= Hi*delta     %如果中心位置的食物浓度比当前位置高，并且不拥挤，则向中心位置走一步      PS：H值越小，则离目标越近
        ...
        for i = 1:1:try_number
            ...
            Xnext = Xi + ((Xmin-Xi)+(BX-Xi))/(norm((Xmin-Xi)+(BX-Xi))) * rightInf * rand();
